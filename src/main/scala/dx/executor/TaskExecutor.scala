...
import wdlTools.exec.SafeLocalizationDisambiguator
...
abstract class TaskExecutor(jobMeta: JobMeta,
  ...
  private def readEnv(): (Map[String, TSchema],
    ...
    val schemas = TypeSerde.deserializeMap(schemasJs, decodeDots = false)._2
        ...
        case ((schemaAccu, paramAccu), (key, obj: JsObject)) =>
          obj.getFields("type", "value") match {
            case Seq(typeJs, valueJs) =>
              val (irType, newSchemas) = TypeSerde.deserializeOne(typeJs, schemaAccu)
  ...
  def prolog(): Unit = {
    ...
    val (localFilesToPath, filesToStream, filesToDownload) =
      inputs.foldLeft(
        ...
        case ((localFilesToPath, filesToStream, filesToDownload), (name, (_, irValue))) =>
          ...
          if (streamFiles == StreamFiles.None ||
              (streamFiles == StreamFiles.PerFile && !streamFileForInput(name)) ||
              remote.headOption.forall(file => Archive.isArchiveFile(file.name))) {
            // archive filles cannot be streamed
    ...
    val downloadLocalizer =
      SafeLocalizationDisambiguator(
          ...
          disambiguationDirLimit = WdlTaskExecutor.MaxDisambiguationDirs
      )

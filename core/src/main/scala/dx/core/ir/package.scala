...
/** Compile-time representation of an input or output parameter.
  ...
  *   default value
  * @param attributes
  *   metadata used to encode DNAnexus applet input/output specification fields, such as {help,
...
/** A unified type representing a workflow, app, or applet. This is useful when compiling WDL
  * workflows, because they can call other WDL workflows and applets. This is done using the same
  * syntax.
  */
trait Callable {
  def name: String
...
/** @param dependencies:
  *   the order in which to compile the workflows and tasks. The first element in the vector depends
  *   on nothing else. Each other element (may) depend on any of the previous elements.
  */
...
/** Kinds of executables that might be generated: Native: a native platform app or applet Task: call
  * a task, execute a shell command (usually) WfFragment: WDL workflow fragment, can included nested
  * if/scatter blocks WfInputs handle workflow inputs for unlocked workflows WfOutputs: evaluate
  * workflow outputs WorkflowOutputReorg: move intermediate result files to a subdirectory.
  */
...
/** An applet that executes a workflow fragment.
  * @param call
  *   name of the call made in the fragment
  * @param blockPath
  *   path to the block represented by this fragment
  * @param inputs
  *   mapping of input name to type, where names are encoded such that any dots are replaced with
  *   '\_\_\_'
  * @param scatterChunkSize
  *   maximum number of scatter jobs that can be run at the same time
  */
...
/** An app or applet.
  ...
  *   output arguments
  * @param instanceType
  *   a platform instance name
  * @param container
  *   is docker used? if so, what image
  * @param kind
  *   kind of application: task, scatter, ...
  * @param document
  *   task definition
  * @param attributes
  *   additional applet metadata
  * @param requirements
...
/** An input to a stage. Could be empty, a wdl constant, a link to an output variable from another
  * stage, or a workflow input.
  */
...
/** A stage can call an application or a workflow.
  * @note
  *   the description may contain dots, parentheses, and other special symbols. It is shown to the
  *   user on the UI. The dxStage.id is unique across the workflow.
  */
...
/** A workflow output is linked to the stage that generated it.
  *
  * If [level] is SubWorkflow, then a workflow matches part of a WDL workflow, it is not a first
  * class citizen. It is compiled into a hidden dx:workflow.
  */

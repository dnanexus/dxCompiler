...
object RunSpec {
  ...
  /** Specification of instance type. An instance could be: Default: the platform default, useful
    * for auxiliary calculations. Static: instance type is known at compile time. We can start the
    * job directly on the correct instance type. Dynamic: WDL specifies a calculation for the
    * instance type, based on information known only at runtime. The generated app(let) will need to
    * evalulate the expressions at runtime, and then start another job on the correct instance type.
    */
  ...
  /** A task may specify a container image to run under. Currently, DNAnexus only supports Docker
    * images. There are four supported options: None: no image Dynamic: Image determined dynamically
    * at runtime, may reside on a network site or on the platform Network: the image resides on a
    * network site and requires download DxFile: the image is a platform file TODO: add a collection
    * type that can contain multiple ContainerImage values
    */

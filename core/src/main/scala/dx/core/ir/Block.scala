...
/** The kind of block input variable being referenced. A Computed input is one that is computed from
  * other inputs - currently this is only used for the scatter variable.
  */
...
/** These are the kinds of blocks that are run by the workflow-fragment-runner. A block can have
  * expressions, input ports, and output ports in the beginning. The block can be one of these
  * types:
...
trait Block[Self <: Block[Self]] { this: Self =>
  ...
  /** Create a human readable name for a block of statements
    *   1. Ignore all private variables 2. If there is a scatter/if, use that 3. if there is at
    *      least one call, use the first one. 4. If the entire block is made up of expressions,
    *      return None
    * @return
    */
  ...
  def getSubBlockAt(path: Vector[Int])(implicit tag: ClassTag[Self]): Self = {
    path.foldLeft(this) { case (subBlock: Self, index) =>
      subBlock.getSubBlock(index)
    }
  }

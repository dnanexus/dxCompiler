...
label: "Adds an INFO tag (VALIDATED) flagging variants in the pipeline vcf present\
  \ in a previously validated vcf file"
requirements:
...
inputs:
  vcf:
    ...
    inputBinding:
      position: 1
    doc: "Each variant in this file that is also in the validated vcf file (if supplied)\
      \ will be marked with a VALIDATED flag in its INFO field"
  validated_variants:
    type: File?
    secondaryFiles: [.tbi]
    inputBinding:
      position: 2
    doc: "A vcf of previously discovered variants to be marked in the pipeline vcf;\
      \ if not provided, this tool does nothing but rename the input vcf"

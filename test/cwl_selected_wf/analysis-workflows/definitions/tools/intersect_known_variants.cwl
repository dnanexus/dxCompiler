...
label: "Intersect passing validated variants and passing pipeline variants for use\
  \ in pvacseq"
requirements:
...
inputs:
  vcf:
    ...
    inputBinding:
      position: 1
    doc: "Pipeline variants to be intersected with validated variants, if the vcf\
      \ is present"
  validated_variants:
    type: File?
    secondaryFiles: [.tbi]
    inputBinding:
      position: 2
    doc: "A vcf of previously discovered variants; if not provided, this tool does\
      \ nothing but rename the input vcf"

...
inputs:
  ...
  bqsr_known_sites:
    type: File[]
    secondaryFiles: [.tbi]
    label: "bqsr_known_sites: One or more databases of known polymorphic sites used\
      \ to exclude regions around known polymorphisms from analysis."
    doc: |
  ...
  bqsr_intervals:
    type: string[]
    label: "bqsr_intervals: Array of strings specifying regions for base quality score\
      \ recalibration"
    doc: |
  ...
  target_intervals:
    type: File
    label: "target_intervals: interval_list file of targets used in the sequencing\
      \ experiment"
    doc: |
  ...
  target_interval_padding:
    type: int
    label: "target_interval_padding: number of bp flanking each target region in which\
  ...
  per_base_intervals:
    ...
    label: "per_base_intervals: additional intervals over which to summarize coverage/QC\
      \ at a per-base resolution"
    doc: "per_base_intervals is a list of regions (in interval_list format) over which\
      \ to summarize coverage/QC at a per-base resolution."
  per_target_intervals:
    type: ../types/labelled_file.yml#labelled_file[]
    label: "per_target_intervals: additional intervals over which to summarize coverage/QC\
      \ at a per-target resolution"
    doc: "per_target_intervals list of regions (in interval_list format) over which\
      \ to summarize coverage/QC at a per-target resolution."
  summary_intervals:
    type: ../types/labelled_file.yml#labelled_file[]
  omni_vcf:
    type: File
  ...
  scatter_count:
    type: int
    doc: "scatters each supported variant detector (varscan, pindel, mutect) into\
      \ this many parallel jobs"
  ...
  docm_vcf:
    type: File
    secondaryFiles: [.tbi]
    doc: "The set of alleles that gatk haplotype caller will use to force-call regardless\
      \ of evidence"
  ...
  filter_somatic_llr_threshold:
    type: float
    default: 5
    doc: "Sets the stringency (log-likelihood ratio) used to filter out non-somatic\
      \ variants.  Typical values are 10=high stringency, 5=normal, 3=low stringency.\
      \ Low stringency may be desirable when read depths are low (as in WGS) or when\
      \ tumor samples are impure."
  filter_somatic_llr_tumor_purity:
    type: float
    default: 1
    doc: "Sets the purity of the tumor used in the somatic llr filter, used to remove\
      \ non-somatic variants. Probably only needs to be adjusted for low-purity (<\
      \ 50%).  Range is 0 to 1"
  filter_somatic_llr_normal_contamination_rate:
    type: float
    default: 0
    doc: "Sets the fraction of tumor present in the normal sample (range 0 to 1),\
      \ used in the somatic llr filter. Useful for heavily contaminated adjacent normals.\
      \ Range is 0 to 1"
  vep_cache_dir:
    type:
    - string
  ...
  vep_ensembl_species:
    type: string
    doc: "ensembl species - Must be present in the cache directory. Examples: homo_sapiens\
      \ or mus_musculus"
  synonyms_file:
    type: File?
    doc: "synonyms_file allows the use of different chromosome identifiers in vep\
      \ inputs or annotation files (cache, database, GFF, custom file, fasta). File\
      \ should be tab-delimited with the primary identifier in column 1 and the synonym\
      \ in column 2."
  annotate_coding_only:
    type: boolean?
    doc: "if set to true, vep only returns consequences that fall in the coding regions\
      \ of transcripts"
  vep_pick:
    type:
    - "null"
    - type: enum
      symbols: ["pick", "flag_pick", "pick_allele", "per_gene", "pick_allele_gene",
        "flag_pick_allele", "flag_pick_allele_gene"]
    doc: "configures how vep will annotate genomic features that each variant overlaps;\
      \ for a detailed description of each option see https://useast.ensembl.org/info/docs/tools/vep/script/vep_other.html#pick_allele_gene_eg"
  cle_vcf_filter:
    type: boolean
    default: false
  variants_to_table_fields:
    ...
    doc: "The names of one or more standard VCF fields or INFO fields to include in\
      \ the output table"
  ...
  manta_call_regions:
    type: File?
    secondaryFiles: [.tbi]
    doc: "bgzip-compressed, tabix-indexed BED file specifiying regions to which manta\
      \ structural variant analysis is limited"
  manta_non_wgs:
    type: boolean?
    default: true
    doc: "toggles on or off manta settings for WES vs. WGS mode for structural variant\
      \ detection"
  manta_output_contigs:
    type: boolean?
    doc: "if set to true configures manta to output assembled contig sequences in\
      \ the final VCF files"
  somalier_vcf:
    type: File
    doc: "a vcf file of known polymorphic sites for somalier to compare normal and\
      \ tumor samples for identity; sites files can be found at: https://github.com/brentp/somalier/releases"
  tumor_sample_name:
    type: string
  normal_sample_name:
    type: string
  validated_variants:
    type: File?
    secondaryFiles: [.tbi]
    doc: "An optional VCF with variants that will be flagged as 'VALIDATED' if found\
      \ in this pipeline's main output VCF"
  cnvkit_target_average_size:
    type: int?
    doc: "approximate size of split target bins for CNVkit; if not set a suitable\
      \ window size will be set by CNVkit automatically"
steps:
  tumor_alignment_and_qc:
    ...
    in:
      ...
      final_name:
        source: tumor_name
        valueFrom: "$(self).bam"
    out: [bam, mark_duplicates_metrics, insert_size_metrics, alignment_summary_metrics,
      hs_metrics, per_target_coverage_metrics, per_target_hs_metrics, per_base_coverage_metrics,
      per_base_hs_metrics, summary_hs_metrics, flagstats, verify_bam_id_metrics, verify_bam_id_depth]
  ...
  detect_variants:
    ...
    out: [mutect_unfiltered_vcf, mutect_filtered_vcf, strelka_unfiltered_vcf, strelka_filtered_vcf,
      varscan_unfiltered_vcf, varscan_filtered_vcf, pindel_unfiltered_vcf, pindel_filtered_vcf,
      docm_filtered_vcf, final_vcf, final_filtered_vcf, final_tsv, vep_summary, tumor_snv_bam_readcount_tsv,
      tumor_indel_bam_readcount_tsv, normal_snv_bam_readcount_tsv, normal_indel_bam_readcount_tsv]
  cnvkit:
    ...
    out: [intervals_antitarget, intervals_target, normal_antitarget_coverage, normal_target_coverage,
      reference_coverage, cn_diagram, cn_scatter_plot, tumor_antitarget_coverage,
      tumor_target_coverage, tumor_bin_level_ratios, tumor_segmented_ratios]
  manta:

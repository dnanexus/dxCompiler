...
inputs:
  ...
  docm_vcf:
    type: File
    secondaryFiles: [.tbi]
    doc: "Common mutations in cancer that will be genotyped and passed through into\
      \ the merged VCF if they have even low-level evidence of a mutation (by default,\
      \ marked with filter DOCM_ONLY)"
  filter_docm_variants:
    type: boolean?
    default: true
    doc: "Determines whether variants found only via genotyping of DOCM sites will\
      \ be filtered (as DOCM_ONLY) or passed through as variant calls"
  vep_cache_dir:
    type:
    - string
    - Directory
  ...
  clinical_mhc_classI_alleles:
    type: string[]?
    label: "Clinical HLA typing results, limited to MHC Class I alleles; element format:\
      \ HLA-X*01:02[/HLA-X...]"
    doc: "used to provide clinical HLA typing results in the format HLA-X*01:02[/HLA-X...]\
      \ when available."
  clinical_mhc_classII_alleles:
    type: string[]?
    label: "Clinical HLA typing results, limited to MHC Class II alleles"
    doc: "used to provide clinical HLA typing results; separated from class I due\
  ...
  net_chop_method:
    ...
    - type: enum
      symbols: ["cterm", "20s"]
    label: "net_chop_method: NetChop prediction method to use ('cterm' for C term\
      \ 3.0, '20s' for 20S 3.0)"
    doc: |
  ...
  run_reference_proteome_similarity:
    type: boolean?
    label: "run_reference_proteome_similarity: sets an option whether to run reference\
      \ proteome similarity or not"
    doc: |
...
steps:
  rnaseq:
    ...
    out: [final_bam, stringtie_transcript_gtf, stringtie_gene_expression_tsv, transcript_abundance_tsv,
      transcript_abundance_h5, gene_abundance, metrics, chart, fusion_evidence, bamcoverage_bigwig]
  somatic:
    ...
    out: [tumor_cram, tumor_mark_duplicates_metrics, tumor_insert_size_metrics, tumor_alignment_summary_metrics,
      tumor_hs_metrics, tumor_per_target_coverage_metrics, tumor_per_target_hs_metrics,
      tumor_per_base_coverage_metrics, tumor_per_base_hs_metrics, tumor_summary_hs_metrics,
      tumor_flagstats, tumor_verify_bam_id_metrics, tumor_verify_bam_id_depth, normal_cram,
      normal_mark_duplicates_metrics, normal_insert_size_metrics, normal_alignment_summary_metrics,
      normal_hs_metrics, normal_per_target_coverage_metrics, normal_per_target_hs_metrics,
      normal_per_base_coverage_metrics, normal_per_base_hs_metrics, normal_summary_hs_metrics,
      normal_flagstats, normal_verify_bam_id_metrics, normal_verify_bam_id_depth,
      mutect_unfiltered_vcf, mutect_filtered_vcf, strelka_unfiltered_vcf, strelka_filtered_vcf,
      ...
      intervals_antitarget, intervals_target, normal_antitarget_coverage, normal_target_coverage,
      reference_coverage, cn_diagram, cn_scatter_plot, tumor_antitarget_coverage,
      tumor_target_coverage, tumor_bin_level_ratios, tumor_segmented_ratios, diploid_variants,
      somatic_variants, all_candidates, small_candidates, tumor_only_variants, somalier_concordance_metrics,
  ...
  germline:
    ...
    out: [cram, mark_duplicates_metrics, insert_size_metrics, insert_size_histogram,
      alignment_summary_metrics, hs_metrics, per_target_coverage_metrics, per_target_hs_metrics,
      per_base_coverage_metrics, per_base_hs_metrics, summary_hs_metrics, flagstats,
      verify_bam_id_metrics, verify_bam_id_depth, raw_vcf, final_vcf, filtered_vcf,
      vep_summary, optitype_tsv, optitype_plot]
  phase_vcf:
...
outputs:
  ...
  tumor_per_target_coverage_metrics:
    ...
    label: "Sequencing per-target coverage summary of target intervals from tumor\
      \ DNA"
    doc: |
  ...
  normal_per_target_coverage_metrics:
    ...
    label: "Sequencing per-target coverage summary of target intervals from normal\
      \ DNA"
    doc: |

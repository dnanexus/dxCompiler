...
case class HintResolver(wdlVersion: WdlVersion,
  ...
  def getInput(name: String): Option[WdlValues.V] = {
    parameterMeta.get(name).orElse(hintInputs.flatMap(_.get(name)))
  }

  def getOutput(name: String): Option[WdlValues.V] = {
    parameterMeta.get(name).orElse(hintOutputs.flatMap(_.get(name)))
  }

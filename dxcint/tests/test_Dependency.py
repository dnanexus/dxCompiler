...
def test_dependency_factory_package(dependency_conf_dir, context_init):
    dependency_factory = DependencyFactory(
        os.path.join(dependency_conf_dir, "mock_dependency.json"), context_init
    )
...
def test_dependency_factory_binary(
    dependency_conf_dir, link_dest_and_cleanup, context_init
):
    dependency_factory = DependencyFactory(
        os.path.join(dependency_conf_dir, "mock_dependency_src.json"), context_init
    )
...
def test_dependency_factory_tarball(
    dependency_conf_dir, link_dest_and_cleanup, context_init
):
    dependency_factory = DependencyFactory(
        os.path.join(dependency_conf_dir, "mock_dependency_tar.json"), context_init
    )

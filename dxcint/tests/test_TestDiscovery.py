...
def add_test_to_existing_category(fixtures_dir, context_init):
    with open(
        os.path.join(fixtures_dir, "config", "medium.json"), "r"
    ) as old_suite_handle:
    ...
    added_tests = test_discovery.add_tests(
        dir_name=new_tests_dir, extension="wdl", suite="M", category="mock_category"
    )
    yield added_tests
    with open(
        os.path.join(fixtures_dir, "config", "medium.json"), "w"
    ) as modified_suite_handle:
...
def add_test_to_new_category(fixtures_dir, context_init):
    ...
    added_tests = test_discovery.add_tests(
        dir_name=new_tests_dir, extension="wdl", suite="M", category="new_mock_category"
    )
    yield added_tests
    with open(
        os.path.join(fixtures_dir, "config", "medium.json"), "w"
    ...
    for added_file in os.listdir(new_tests_dir):
        os.remove(
            os.path.join(fixtures_dir, "resources", "new_mock_category", added_file)
        )
...
def test_add_tests(
    add_test_to_existing_category, add_test_to_new_category, fixtures_dir
):
